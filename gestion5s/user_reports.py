...
SOURCES = (
    {"entity": "reclamos", "label": "Reclamos usuarios", "date": "fecha", "date_label": "Fecha",
     "status": "estatus", "sheet": "Reclamos usuarios", "expected_open": ("abierto",)},
    ...
    {"entity": "samtech_usuarios", "label": "Samtech usuarios", "date": "fecha_creacion",
     ...
     "expected_open": ("no_iniciada", "en_progreso")},
)
STATUS_LABELS = {
...
METHOD_NOTE = (
    "Los casos se agrupan por la fecha del registro y su estado actual. "
...
def build_user_report(db, models, start, end, include_details=False):
    ...
    for source in SOURCES:
        ...
        date_column, status_column = getattr(model, source["date"]), getattr(model, source["status"])
        ...
        if include_details:
            ...
            for record in db.query(model).filter(*scope).order_by(date_column, model.id).yield_per(1000):
                grouped[(getattr(record, source["date"]), getattr(record, source["status"]))] += 1
        ...
        totals["rate"] = totals["closed"] / denominator if denominator else 0
        categories.append({
    ...
    closed_rows = [row(c["entity"] + "_closed", f"{c['label']} (Cerrado)", c["counts"]["closed"]) for c in categories]
    ...
    if totals["unclassified"]:
        ...
        sections.append({"key": "unclassified", "title": "Estados vacíos o sin clasificar", "rows": unknown_rows})
    comparison = [
...
def make_conclusions(report):
    ...
    if totals["unclassified"]:
        conclusions.append(f"Revisar estados: {cases(totals['unclassified'])} del período "
                           f"{'requiere' if totals['unclassified'] == 1 else 'requieren'} clasificación. "
